...
COMPONENT average_instance_P1_fixpt
 ...
 SIGNAL deinterlace_en                      : std_logic_vector(3 DOWNTO 0); 
 SIGNAL AVG_en                              : std_logic_vector(3 DOWNTO 0); 
...
BEGIN
    ...
    if (rising_edge(clk)) then
        ...
        deinterlace_en(1)   <= sfft_en(0);
        deinterlace_en(2)   <= deinterlace_en(1);
        deinterlace_en(3)   <= deinterlace_en(2);
        
        AVG_en(1)           <=  deinterlace_en(0);
        AVG_en(2)           <=  AVG_en(1); 
        AVG_en(3)           <=  AVG_en(2); 
 ...
 blk_reset  <= reset or (not nstart_r);
 ...
 deinterlace_en(0)  <= sfft_en(0)           when (deinterlace_DLYr = x"0")  else
                       deinterlace_en(1)    when (deinterlace_DLYr = x"1")  else
                       deinterlace_en(2)    when (deinterlace_DLYr = x"2")  else
                       deinterlace_en(3)    when (deinterlace_DLYr = x"3")  else
                       sfft_en(0);
 
 
 AVG_en(0)          <= deinterlace_en(0)  when (AVG_DLYr = x"0")  else
                       AVG_en(1)         when (AVG_DLYr = x"1")  else
                       AVG_en(2)         when (AVG_DLYr = x"2")  else
                       AVG_en(3)         when (AVG_DLYr = x"3")  else
                       deinterlace_en(0);
...
deinterlace_instance_12_fixpt_inst : deinterlace_instance_12_fixpt
 ...
 average_instance_P1_fixpt_inst : average_instance_P1_fixpt
  PORT map
        ...
        --ch1_val_re           => ch1_val_re,
        --ch1_val_im           => ch1_val_im,
        --ch2_val_re           => ch1_val_re,
        --ch2_val_im           => ch1_val_im,
        --count                => bin,
        --ready_in             => fft_ready,   
   ...
   average_instance_P1_fixpt_inst2 : average_instance_P2_fixpt
  PORT map
        ( clk                => clk,
        reset                => blk_reset,
        clk_enable           => AVG_en(0),
